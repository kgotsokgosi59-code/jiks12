...
(function () {
  ...
  // Licence gate: a track only plays if it has a file AND its licence is cleared.
  // A file with no licence, or a pending/withdrawn one, is never played — the
  // licence record in playlist.js is enforced here, not just written down.
  var tracks = list.filter(function (t) {
    return t && t.file && t.licence && t.licence.status === 'cleared';
  });
  ...
  function load(i, keepTime) {
    ...
    if (resumeAt) {
      audio.addEventListener('loadedmetadata', function once() {
        ...
        try { audio.currentTime = resumeAt; } catch (e) { /* ignore */ }
      });
    }
  }
  ...
  function play() {
    ...
    if (p && typeof p.catch === 'function') {
      p.catch(function () {
        // Blocked by the browser, or the file isn't there. Fall back to paused,
        // and if we were mid-session, resume on the visitor's first interaction.
        setState(false);
        if (pendingResume) {
  ...
  // Browsers only allow sound after the visitor has interacted with the page, so
  // if a resume was blocked we wait for their next click or key and try again.
  function armResume() {
    ...
    events.forEach(function (e) { window.removeEventListener(e, retry); });
    events.forEach(function (e) { window.addEventListener(e, retry); });
  }
  ...
  audio.addEventListener('play', function () { pendingResume = false; setState(true); saveState(); });
  audio.addEventListener('pause', function () { setState(false); saveState(); });
  audio.addEventListener('ended', function () { load(index + 1, false); play(); });
  ...
  function start() {
    ...
    var startAt = (typeof saved.i === 'number' && saved.i < tracks.length) ? saved.i : 0;
    load(startAt, true);
    setState(false);
  ...
  if (window.fetch) {
    fetch(tracks[0].file, { method: 'HEAD' }).then(function (res) {
      if (res.ok) start();
